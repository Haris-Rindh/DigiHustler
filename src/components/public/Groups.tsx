            ...
            <div key={group.id} className="glass-card p-8 rounded-3xl space-y-6">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[var(--border-subtle)] pb-6">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h2 className="font-display font-extrabold text-2xl text-[var(--text-heading)]">{group.name}</h2>
              ...
              {/* Skill Tags */}
              <div>
                <h4 className="text-xs font-bold text-[var(--text-body)] uppercase tracking-wider mb-2">Group Specialties & Technical Stack</h4>
                <div className="flex flex-wrap gap-2">
                  {group.specialties.map((spec, i) => (
                    <span key={i} className="text-xs px-3 py-1 rounded-xl bg-[var(--bg-subtle)] text-[var(--text-body)] border border-[var(--border-subtle)] font-medium">
                      {spec}
                    </span>
                  ))}
                </div>
