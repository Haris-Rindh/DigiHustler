...
export const ProjectDetailModal: React.FC<Props> = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-slate-900 border border-white/10 rounded-3xl shadow-2xl p-6 sm:p-8 my-8 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-6">
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-slate-400">• Group: {activeGroup.name}</span>
            </div>
            <h2 className="font-display font-extrabold text-2xl text-white mt-1">{project.title}</h2>
          </div>
        ...
        {/* Tab Selection */}
        <div className="flex space-x-2 border-b border-white/10 pb-3 mb-6">
          <button 
            onClick={() => setActiveTab('overview')} 
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'overview' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Brief & Financial Split
          </button>
          <button 
            onClick={() => setActiveTab('team')} 
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'team' ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Team Assignment ({project.assignments.length})
          </button>
          <button 
            onClick={() => setActiveTab('deliverables')} 
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'deliverables' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Deliverables ({project.deliverables.length})
          </button>
          <button 
            onClick={() => setActiveTab('comments')} 
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'comments' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white/5 p-4 rounded-2xl border border-white/10">
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-400">Client Info</span>
                <p className="text-sm font-bold text-white mt-0.5">{project.clientName}</p>
                <p className="text-xs text-slate-400">{project.clientEmail}</p>
              </div>
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-400">Department Group</span>
                <p className="text-sm font-bold text-cyan-400 mt-0.5">{activeGroup.name}</p>
                <p className="text-xs text-slate-400">Leader: {project.assignedLeaderName}</p>
              </div>
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-400">Gross Budget</span>
                <p className="text-sm font-bold text-emerald-400 mt-0.5">${project.totalValue?.toLocaleString() || 0} USD</p>
                <p className="text-xs text-slate-400">Net Revenue: ${project.netRevenue?.toLocaleString() || 0}</p>
              </div>
            </div>

            {/* Scope Brief */}
            <div className="p-4 rounded-2xl bg-slate-950 border border-white/10 space-y-1">
              <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Project Scope Brief</span>
              <p className="text-sm text-slate-200 leading-relaxed whitespace-pre-line">{project.brief}</p>
            </div>
              ...
              {currentUser.role === 'management' ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-slate-300 mb-1 block">Gross Total Value ($)</label>
                      <input 
                        type="number" 
                        value={totalVal}
                        onChange={(e) => setTotalVal(parseFloat(e.target.value) || 0)}
                        className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-1.5 text-sm text-white focus:border-cyan-400 focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-slate-300 mb-1 block">Assign Department Leader</label>
                      <select
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="text-xs font-semibold text-slate-300 mb-1 flex justify-between">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div className="p-3 rounded-xl bg-purple-500/10 border border-purple-500/20">
                    ...
                    <p className="text-lg font-bold text-white mt-1">{project.splitManagementPct}%</p>
                  ...
                  <div className="p-3 rounded-xl bg-cyan-500/10 border border-cyan-500/20">
                    ...
                    <p className="text-lg font-bold text-white mt-1">{project.splitLeaderPct}%</p>
                  ...
                  <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                    ...
                    <p className="text-lg font-bold text-white mt-1">{project.splitFreelancerPct}%</p>
            ...
            {project.status === 'completed' && (
              <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-500/15 via-teal-500/15 to-cyan-500/15 border border-emerald-500/30 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-300">Release payouts according to stored split percentage rules.</p>
                </div>
                <button
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400">Assign single freelancer or split the Freelancer Pool among multiple specialists</p>
              </div>
            ...
            <div className="space-y-3">
              {assignments.map((assignment, index) => (
                <div key={index} className="p-4 rounded-2xl bg-slate-950 border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex-1 w-full space-y-2">
                    <label className="text-xs font-semibold text-slate-400">Select Roster Member</label>
                    <select
                      ...
                      className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:border-cyan-400 focus:outline-none"
                    >
                  ...
                  <div className="flex-1 w-full space-y-2">
                    <label className="text-xs font-semibold text-slate-400">Role Title on Project</label>
                    <input 
                      type="text"
                  ...
                  <div className="w-32 space-y-2">
                    <label className="text-xs font-semibold text-slate-400">Share of Pool %</label>
                    <input 
                      type="number"
                      min="0"
                      max="100"
                      ...
                      className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-sm text-white text-center focus:border-cyan-400 focus:outline-none font-bold"
                    />
                  </div>
                </div>
              ))}
        ...
        {activeTab === 'deliverables' && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              {project.deliverables.length === 0 ? (
                ...
                project.deliverables.map(del => (
                  <div key={del.id} className="p-4 rounded-2xl bg-slate-950 border border-white/10 flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-bold text-white">{del.title}</h4>
                      <p className="text-xs text-slate-400">Submitted by: {del.submittedByUserName} on {new Date(del.submittedAt).toLocaleDateString()}</p>
                      {del.notes && <p className="text-xs text-slate-300 mt-1 italic">"{del.notes}"</p>}
                    </div>
                    {del.linkUrl && (
                      <a 
                        href={del.linkUrl} 
            ...
            <form onSubmit={handleAddDeliverable} className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3">
              <h4 className="text-xs font-bold text-white uppercase tracking-wider">Submit New Deliverable</h4>
              ...
              <textarea 
                ...
                className="w-full bg-slate-950 border border-white/10 rounded-xl p-2 text-xs text-white focus:outline-none focus:border-cyan-400"
              />
              <button 
                type="submit"
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
              {project.comments.length === 0 ? (
                <p className="text-center text-xs text-slate-400 py-6">No discussions yet. Start the conversation below!</p>
              ) : (
                project.comments.map(c => (
                  <div key={c.id} className="p-3.5 rounded-2xl bg-slate-950 border border-white/10 flex space-x-3">
            ...
            <form onSubmit={handleAddComment} className="flex space-x-2">
              <input 
                ...
                className="flex-1 bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-xs text-white focus:outline-none focus:border-cyan-400"
              />
              <button 
                type="submit"
