...
export const Footer: React.FC = () => {
  return (
    <footer className="bg-[var(--color-bg)] border-t border-[var(--color-border)] pt-16 pb-10 px-6 lg:px-8 relative overflow-hidden" aria-label="Site Footer">
      {/* Subtle top ambient border */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-4xl h-px bg-gradient-to-r from-transparent via-[var(--color-border)] to-transparent pointer-events-none" />

      <div className="max-w-7xl mx-auto">

        {/* Top Grid */}
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-10 pb-12 border-b border-[var(--color-border)]">
          ...
          <div className="md:col-span-2 space-y-4">
            <Link to="/" className="flex items-center space-x-2.5 group">
              ...
              <span className="font-display font-extrabold text-xl text-white group-hover:text-[var(--color-text-primary)] transition-colors">
                DigiHust
              </span>
            </Link>
            <p className="text-xs font-extrabold text-[var(--color-text-primary)] uppercase tracking-widest">
            ...
            <div className="flex items-center space-x-2.5 pt-2">
              {[
                ...
                <motion.a
                  ...
                  title={s.label}
                  aria-label={s.label}
                  className="w-9 h-9 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] hover:border-[var(--color-accent)] hover:bg-[var(--color-accent-fill)]/20 flex items-center justify-center text-xs font-bold text-slate-300 hover:text-[var(--color-text-primary)] transition-colors shadow-sm"
                >
                  {s.icon}
                </motion.a>
              ))}
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2.5 text-sm text-slate-400">
              {[
                ...
                <li key={s.name}>
                  <Link to={s.href} className="hover:text-[var(--color-text-primary)] transition-colors inline-flex items-center space-x-1 group">
                    <span>{s.name}</span>
                  </Link>
                </li>
              ))}
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2.5 text-sm text-slate-400">
              {[
                ...
                <li key={link.label}>
                  <Link to={link.href} className="hover:text-[var(--color-text-primary)] transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2.5 text-sm text-slate-400">
              <li>
                <Link to="/dashboard" className="text-[var(--color-text-primary)] hover:underline font-bold inline-flex items-center space-x-1">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 text-xs text-slate-500">
          ...
          <div className="flex items-center space-x-4">
            ...
            <span>·</span>
            <Link to="/contact" className="text-[var(--color-text-primary)] hover:underline">Support & Inquiries</Link>
          </div>
        </div>
